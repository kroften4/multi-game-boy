...
?#link("https://github.com/Bodmer/TFT_eSPI")[TFT_eSPI] библиотека
(👍 для ESP32 поддержка 8bit-parallel без DMA)
(нужен дисплей с контроллером который поддерживается ей)
...
https://youtu.be/LSMQ3U1Thzw?si=DHHx-LF8a_1jlMix
...
== Network connection
Подключение по WiFi (Можно развернуть удаленно сервер / по локальной сети,
сервер на одной из приставок/отдельном девайсе)
